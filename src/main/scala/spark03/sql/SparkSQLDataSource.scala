...
object SparkSQLDataSource {
  ...
  private def runBasicDataSourceExample(spark:SparkSession): Unit ={
    ...
    val peopleDF = spark.read.format("json").load("file:\\D:\\mycode1\\BigData\\hello_bigdata\\data\\people.json")
    peopleDF.select("name","age").write.format("parquet").save("namesAndAge.parquet")

    val sqlDF = spark.sql("select * from parquet.`file:\\D:\\mycode1\\BigData\\hello_bigdata\\data\\users.parquet`")
  ...
  private def runParquetSchemaMergingExample(spark:SparkSession): Unit ={
    ...
    // The final schema consists of all 3 columns in the Parquet files together with partitioning column appeared in the partition directory paths .
